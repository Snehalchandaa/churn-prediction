...
from retention import get_action, get_reason, recommend_products
from email_templates import generate_email

# -------------------------------
# LOAD FILES
# -------------------------------
model = pickle.load(open("d:/Mokbul/projects/churn_prediction/models/model.pkl", "rb"))
features = pickle.load(open("d:/Mokbul/projects/churn_prediction/models/features.pkl", "rb"))
imp_df = pd.read_csv("d:/Mokbul/projects/churn_prediction/models/feature_importance.csv")
...
if st.session_state.prediction_done:
    ...
    strategy = get_action(prob, clv, engagement, cart)

    st.subheader("Retention Strategy")

    colA, colB = st.columns(2)

    colA.metric("Customer Segment", strategy["segment"])
    colB.metric("Priority Level", strategy["priority"])

    st.success(f"Offer: {strategy['offer']}")
    st.info(f"Channel: {strategy['channel']}")
    st.write(f"Campaign: {strategy['campaign']}")

    # Campaign trigger simulation
    if prob > 0.6:
        st.warning("Campaign Triggered Automatically")
    ...
    df_all = pd.read_csv("d:/Mokbul/projects/churn_prediction/models/final_data.csv")
    ...
    # CHURN REASON
    # -------------------------------
    reason = get_reason(recency, engagement, cart, email)
    ...
    customers_saved = int((1 - prob) * 100)
    revenue_saved = customers_saved * clv * 0.2

    col1, col2 = st.columns(2)
    col1.metric("Customers Saved", customers_saved)
    col2.metric("Revenue Retained", f"{revenue_saved:.2f}")
    # -------------------------------
    # PRODUCT RECOMMENDATION
    # -------------------------------
    st.subheader("Product Recommendations")

    products = recommend_products(strategy["segment"])
